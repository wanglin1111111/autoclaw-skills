...
## Command Bundling — Safety Guard 审批优化

AutoClaw Safety Guard 对每个 exec 命令独立审批，不可绕过（系统级防篡改）。

### 强制规则

1. **所有需要 exec 的操作必须合并为单一脚本**，一次审批完成全部工作
2. 脚本写在 workspace 临时文件中，exec 一次性执行
3. 禁止逐条 exec —— 每增加一条 exec，多一次审批循环
4. 优先使用 `read` / `write` / `edit` 工具（无需审批），仅在必要时使用 exec
...
✅ 正确（1 次审批）：
```
write: bundle.ps1 (包含所有操作)
exec: powershell -File bundle.ps1
```

### GitHub 推送
- 所有 git 操作合并为一条脚本：add + commit + push
- 使用 `gh` CLI（已认证，用户 wanglin1111111）
- 仓库：https://github.com/wanglin1111111/autoclaw-skills

### 工具可用性
- ✅ `read` / `write` / `edit` — 无需审批
- ❌ `image()` — sharp 模块不可用（Windows）
- ❌ `canvas` — 无 node 设备
- ⚠️ `exec` — 需审批，合并后提交
